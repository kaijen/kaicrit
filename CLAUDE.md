...
| [edit/trackChanges.ts](src/edit/trackChanges.ts) | `TrackChangesManager` — thin VS Code wrapper around the engine. Per-document state (`enabled` set + `shadow` text snapshots, since the change event omits deleted text), a re-entrancy guard so its own compensating `WorkspaceEdit` isn't re-processed; the guard is released in **both** the success and the rejection branch of the `applyEdit` promise, so a single failed `applyEdit` (read-only doc, conflicting edit) can't wedge the recorder permanently — the shadow-sync + caret-restore follow-up runs only when `applyEdit` resolves truthy. Undo/redo left untouched, plus the `$(edit) Track Changes` status-bar item and `kaicrit.trackChanges` context key. `applyDefault` honours the `kaicrit.edit.trackChanges` setting for freshly opened docs. Covered by `trackChanges.test.ts` (guard release on rejection, via the `vscodeStub`'s swappable `applyEdit`) |
