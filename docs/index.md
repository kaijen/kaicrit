...
- **Edit** — insert change markers, navigate between them, and accept or reject changes without leaving the editor — including inline **Accept · Reject** actions, shown on hover by default or as an always-on CodeLens row. Changes are mirrored on the overview ruler, summarized by type in the status bar, and listed in a dedicated [Changes sidebar](overview.md) — which also carries a [Files with Changes overview](overview.md#files-with-changes) of every file that contains CriticMarkup. Editor features run for the [file types you configure](activation.md), with a per-file status-bar toggle.
- **[Track Changes](track-changes.md)** — record edits live as CriticMarkup, like "track changes" in a word processor.
- **[Compare](compare.md)** — diff two files into a single CriticMarkup document.
- **[Double-Pane](doublepane.md)** — view a CriticMarkup document as Original | New side by side, coloured but without the marker syntax.
